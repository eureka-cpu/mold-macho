  ...
  ( finalAttrs:
    ...
    vendorDeps
      ...
      { inherit (manifest.package) name version;
        ...
        checkInputs = [ zlib ];
        # The test suite shells out to Apple's own clang, otool, nm, vtool
        # (via xcrun), codesign, dyld_info and python3 by bare name. Nix's
        # sandboxed PATH only has buildInputs' bin dirs, and nixpkgs' own
        # cctools is a reimplementation whose output doesn't match what the
        # tests expect from Apple's originals, so route the real system
        # tools onto PATH instead of packaging equivalents.
        preCheck =
